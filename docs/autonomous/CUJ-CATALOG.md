...
| CUJ-39 | Edit/rotate an authenticated custom provider's base URL, API mode, or key from the phone | `GET /api/providers` returns `base_url` + `api_mode` for a custom-slug row (auth_type="custom"), NEVER the api_key; tapping the authenticated custom row opens a pre-filled edit form (name locked, key blank); smoke asserts the GET response carries transport metadata for a custom slug and zero credential leakage (`key_env` absent, no `sk-` value, no env-var name) | ABH-257 |
| CUJ-40 | Live turn runs 3+ tool calls without pushing the composer off screen | `smoke: ios-sim` seeds a live assistant turn with 1, then 2, then 3 consecutive tool activities; asserts 1–2 rows expose no `boundedLiveToolWindow`, the 3rd row reveals `boundedLiveToolWindow` with the newest `toolDetailDisclosure` pinned at the bottom/top-faded scrollback, and tapping any `toolDetailDisclosure` removes the bounded window so Arguments + Result are readable flat | ABH-391 |
