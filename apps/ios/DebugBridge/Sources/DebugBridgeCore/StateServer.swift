// Installed from gstack/ios-qa/templates/StateServer.swift.template (task UI-G).
...
// IPv6 tunnel (physical device) or the simulator loopback. It MUST refuse any
// caller without a current bearer token. The boot token is rotated within ~5
// seconds of daemon spawn so anything scraping os_log past that window sees a
// dead credential.
...
// MARK: - Bridges (implementation provided by DebugBridgeUI)
