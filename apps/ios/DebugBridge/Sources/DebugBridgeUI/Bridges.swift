// Installed from gstack/ios-qa/templates/Bridges.swift.template (task UI-G).
//
...
public enum DebugBridgeUIWiring {
    /// Install all three bridge resolvers (and the on-device DebugOverlay).
    /// Idempotent — calling multiple times reinstalls the same closures. Must
    /// be called on @MainActor because every UIKit access requires the main
    /// actor.
    public static func installAll(recording: Bool = false) {
        ...
        DebugOverlayWindow.shared.setAttribution(StateServer.shared.lastAgentIdentity)
    }
}
...
enum MutationBridgeImpl {
    ...
    private static func handleTap(_ payload: JSONDict) -> Bool {
        ...
        // ObjC selector `sendTapAtPoint:inWindow:` imports to Swift as
        // `sendTap(at:in:)`.
