...
final class RelaySessionCoordinator {
    ...
    ///
    /// QA-1 B4: the re-projection of the emptied set is a deliberate FALLBACK
    /// NO-OP on `chatStore.messages` — `applyRelayItems([])` keeps the painted
    /// transcript (the new session's cache seed, or the old content until that
    /// seed lands) instead of blanking it. The old wipe raced the cache seed
    /// and produced a fully blank screen (empty at generation > 0 — no skeleton
    /// either). The relay task-list mirror still clears so the new session's
    /// dock starts clean.
