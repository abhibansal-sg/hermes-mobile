...
// (tui_gateway/server.py + apps/desktop/src/types/hermes.ts).
...
struct SessionSummary: Decodable, Identifiable, Sendable, Equatable {
    ...
    /// a live frame (ABH-86) — re-sorting the row to the top immediately, before
...
/// (`web_server.py:6757-6773`) emits a much richer dict (path, model, provider,
...
/// (`web_server.py:1717-1724`) in ``SessionSummary/profile``. Profiles whose
...
struct SessionOpenResult: Decodable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // (tui_gateway/server.py:3241 `"resumed": target`); session.create
...
struct StoredMessage: Sendable {
    ...
    // session `cron_ebc83e783d98_20260607_155217`) carries the FULL union of
...
struct ClarifyRequestPayload: Sendable, Equatable {
    ...
    /// `tui_gateway/server.py:1117`) and `clarify.respond` routes the answer by
    /// looking it up in `_pending[request_id]` (the generic `_respond`,
    /// server.py:5059). Without it the reply 4009s ("no pending clarify
...
/// (server.py:2122); every key is optional on the wire (older emitters omit the
...
/// Payload of a `sudo.request` event (server.py:2214). The gateway emits an
...
struct SudoRequestPayload: Sendable, Equatable {
    ...
    /// to). Real gateway frames always inject one (`_block`, server.py:1117).
    init?(payload: JSONValue) {
...
/// Payload of a `secret.request` event (server.py:2220). Carries the prompt
...
/// new event and no new `ChatMessage` field, per the contract. Returns `nil`
...
struct TodoList: Sendable, Equatable {
    ...
    /// (`tools/todo_tool.py`, `server.py:2076 if name == "todo"`).
