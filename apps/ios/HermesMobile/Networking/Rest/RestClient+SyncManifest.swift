...
extension RestClient {
    ...
    func syncManifest(scope: CacheScope, cursor: String?) async throws -> SyncManifestPage {
        guard relayControlBaseURL != nil || pathStyle == .plugin else { throw RestError.badStatus(404, body: "sync manifest unavailable") }
        ...
        let data = try await relayControlBaseURL == nil
            ? get(path: "\(mobileAPIPrefix)/sync-manifest?\(query)")
            : getRelayControl(path: "/sync/manifest?\(query)")
