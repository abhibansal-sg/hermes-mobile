...
struct ComposerView: View {
    ...
    /// machinery was unreachable dead code (R1 #18).
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        // ABH-79 Task 3: the outer opaque toolbarBg strip is removed so the
        ...
        .sheet(isPresented: $showFileBrowser) {
            ...
            // The sheet root owns the ONE NavigationStack (R1 #6 — drill-ins
        ...
        .onAppear {
            ...
            // (R1 #92): a race-cancelled salvage never fires this.
    ...
    ///
    /// B1 (ABH-74) — the gesture HOST must survive the field↔strip transition.
    ...
    private var composerCard: some View {
        ...
        // hold recognizer is never torn down — that teardown is the ABH-74 wedge)
    ...
    private var attachButton: some View {
        ...
        //
        // ABH-79 Task 4: replaced the bottom confirmationDialog with an anchored
    ...
    private var modelChip: some View {
        // Prefer the per-session hot-swap model (ABH-84) over the global default.
    ...
    ///
    /// B1 (ABH-74): the tap + long-press now share ONE recognizer tree — a single
    ...
    /// fires.
    ///
    /// B5 (ABH-74 RIDER 4): the mic stays VISIBLE for discoverability but is inert
    ...
    /// drain into a different session opened later (R1 #17); a draft (no
    ...
    /// offline would silently vanish on stop (B5 / RIDER 4). The gate is the MIC
    /// entry point only; it does NOT touch the send/queue path (R1 #18).
    private func tapMic() {
    ...
    /// RIDER 4). MIC entry gate only; the send/queue path is untouched (R1 #18).
...
private struct RecordingStrip: View {
    ...
    /// slide-away to cancel), but B2 (ABH-74) adds the ALWAYS-reachable stop +
