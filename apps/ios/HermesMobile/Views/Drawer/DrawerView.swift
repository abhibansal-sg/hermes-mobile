...
struct DrawerView: View {
    ...
    /// (ABH-86 RIDER — the prior tautological test compared two literals in the
    ...
    /// The session a rename alert is editing (R1 #66 — `SessionStore.rename`
    ...
    /// ABH-80: the session staged for destructive-delete confirmation. Set when
    ...
    var body: some View {
        ...
        // ABH-73: a session mutation (delete/archive/rename) failed. A failure
        ...
        // form. The destructive-confirmation dialog is OUT of scope (ABH-80) —
        ...
        // ABH-80: destructive-delete confirmation dialog. Sits in FRONT of the
        // existing delete path (ABH-73 fix). The `.destructive` role colours the
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            // on today's server (no `/api/profiles`) it is absent entirely and the
    ...
    /// (ABH-80 item 3 — the avatar rendered as a circle with initials; the gear
    ...
    private func rowContextMenu(for summary: SessionSummary, pinned: Bool) -> some View {
        ...
        // R1 #66: rename + archive were fully implemented, REST-wired, and
        ...
        // actual `SessionStore.delete` call (wired in FRONT of the ABH-73 path).
    ...
    /// bottom safe area (~14pt above it) rather than floating high.
    ///
    /// ABH-80 item 1: made taller / more substantial. The pre-26 branch uses
    ...
    /// The footer. ABH-80 item 6: the Quick Note row is removed from the drawer
        ...
        VStack(alignment: .leading, spacing: 3) {
            ...
            HStack(spacing: 6) {
                if let date = result.startedDate {
                    // ABH-86 item 4: tick the relative-time label every 60s.
