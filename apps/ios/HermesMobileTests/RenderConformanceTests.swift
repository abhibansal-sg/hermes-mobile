...
final class RenderConformanceTests: XCTestCase {
    ...
    ///
    /// NOTE for the B8 fix lane: the decision is exposed as the pure static
    /// `ChatView.shouldShowInlineTurnActivity(…)` specifically for this gate —
    /// implement the relay-path suppression clause THERE (extending the
    /// signature if needed; reconcile this call site at integration).
    func testRelay_StreamingBubbleSuppressesInlineWorkingPill() async throws {
        ...
        XCTAssertFalse(
            ChatView.shouldShowInlineTurnActivity(
                ...
                hasPendingApproval: g.chat.pendingApproval != nil,
                hasPendingClarification: g.chat.pendingClarification != nil,
                hasPendingSecurePrompt: g.chat.pendingSecurePrompt != nil
            ),
