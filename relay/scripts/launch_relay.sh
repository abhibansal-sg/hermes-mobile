...
#
#   gateway  ws://127.0.0.1:9133/api/ws?token=   (upstream, stock, isolated)
#   relay    ws://127.0.0.1:8793                 (downstream, phone-facing)
#
...
#   HERMES_RELAY_GATEWAY_PORT      — upstream gateway port (9133 here)
...
RELAY="${RELAY:-$(cd "$HERE/.." && pwd)}"
EVID="${EVID:-/Volumes/MainData/Developer/hermes-tmp/evidence/relay-migration-gate}"
VENV="${RELAY_VENV:-/Volumes/MainData/Developer/hermes-tmp/r0-relay-spike/venv}"
...
export HERMES_RELAY_GATEWAY_PORT="${HERMES_RELAY_GATEWAY_PORT:-9133}"
export HERMES_RELAY_DOWNSTREAM_PORT="${HERMES_RELAY_DOWNSTREAM_PORT:-8793}"
