"""SessionStore LRU bound — regression for the soak T7 I6 finding (D2 port).
...
the marathon flood leaves 4200 states where the cap leaves 4096) and PASS
with the bound.

This is a HAND-PORT of the soak branch's e7aaa19d2 test onto current main
(that branch is 129 commits stale — pre-R4 — so it cannot be merged or
cherry-picked). It reconciles with R4: main's SessionStore additionally holds
the L6 phone-origin prompt markers, so an extra test pins that an evicted
session's transient markers are forgotten with its state.
...
try:
    from hermes_relay.session_state import DEFAULT_SESSION_STORE_CAPACITY
except ImportError:
    # Pre-fix unbounded main has no cap constant; the bound the fix introduces.
...
def test_marathon_flood_stays_bounded_at_default_cap() -> None:
    ...
    assert len(store._states) == DEFAULT_SESSION_STORE_CAPACITY, (
        f"store holds {len(store._states)} session states after a "
        f"{DEFAULT_SESSION_STORE_CAPACITY + 104}-session marathon — expected "
        f"exactly {DEFAULT_SESSION_STORE_CAPACITY} (the unbounded soak I6 leak)"
    )
...
def test_eviction_clears_local_prompt_markers() -> None:
    """R4 reconciliation: an evicted session's L6 phone-origin prompt markers
    are forgotten with its state (both belong to a session the store is done
    remembering; the markers are transient, 120 s TTL'd coordinator state)."""
    store = SessionStore(capacity=4)
    store.mark_local_prompt("sess-000", "hello")
    for i in range(20):
        store.apply(_completed_item_frame(f"sess-{i:03d}", 0))
    assert "sess-000" not in store, "the LRU completed state was not evicted"
    assert store.take_local_prompt("sess-000", "hello") is False, (
        "evicting a session must drop its transient local-prompt markers too"
    )
